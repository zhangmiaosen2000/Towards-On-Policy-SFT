...
class FSDPSFTTrainer:
    def __init__(
        ...
        stats_dir = getattr(self.config.trainer, 'stats_dir', None)
        if stats_dir is None:
            stats_dir = self.config.trainer.default_local_dir
        os.makedirs(stats_dir, exist_ok=True)
        experiment_name = self.config.trainer.experiment_name
        self.stats_save_path = os.path.join(stats_dir, f"{experiment_name}_weight_stats.jsonl")
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        beta = getattr(self.config.model, 'polylog_beta', 1.0)
        gamma_min = getattr(self.config.model, 'polylog_gamma_min', 0.1)
        gamma_max = getattr(self.config.model, 'polylog_gamma_max', 2.0)
        warmup_ratio = getattr(self.config.model, 'polylog_warmup_ratio', 0.2)
        transition_ratio = getattr(self.config.model, 'polylog_transition_ratio', 0.1)
        ablation_phi_bound = getattr(self.config.model, 'ablation_phi_bound', 0.1)
        
        if self.total_training_steps is not None and self.total_training_steps > 0:
            progress = self.global_step / self.total_training_steps
            if progress < warmup_ratio:
                lambda_t = 0.0
            elif progress < warmup_ratio + transition_ratio:
                lambda_t = (progress - warmup_ratio) / transition_ratio
            else:
                lambda_t = 1.0
        else:
            lambda_t = 1.0
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                log_prob = -loss_fct(shift_logits, shift_labels)
                phi_t = log_prob + entropy
                
                gamma_raw = torch.exp(-phi_t / beta)
                gamma_raw = torch.clamp(gamma_raw, gamma_min, gamma_max)
                
                gamma_t = 1.0 + lambda_t * (gamma_raw - 1.0)
                ...
                indomain_mask = ((phi_t > -ablation_phi_bound) & (phi_t < ablation_phi_bound)).float()
                combined_mask = loss_mask.to(polylog_loss_val.device) * indomain_mask.detach()
                
                loss = polylog_loss_val * combined_mask
                
                if self.device_mesh.get_rank() == 0:
                    with torch.no_grad():
                        valid_tokens = loss_mask.bool()
                        indomain_tokens = (combined_mask > 0).bool()
                        gamma_valid = gamma_t[valid_tokens]
                        phi_valid = phi_t[valid_tokens]
                        
                        total_valid_tokens = valid_tokens.sum().item()
                        total_indomain_tokens = indomain_tokens.sum().item()
                        indomain_ratio = total_indomain_tokens / total_valid_tokens if total_valid_tokens > 0 else 0.0
                        
                        gamma_bins = torch.linspace(gamma_min, gamma_max, 11)
                        gamma_bin_counts = torch.histogram(gamma_valid.cpu(), bins=gamma_bins.cpu())[0]
                        gamma_bin_ratios = (gamma_bin_counts / gamma_bin_counts.sum()).tolist() if gamma_bin_counts.sum() > 0 else [0.0]*10
                        
                        phi_range = 3.0 * beta
                        phi_bins = torch.linspace(-phi_range, phi_range, 11)
                        phi_bin_counts = torch.histogram(phi_valid.cpu(), bins=phi_bins.cpu())[0]
                        phi_bin_ratios = (phi_bin_counts / phi_bin_counts.sum()).tolist() if phi_bin_counts.sum() > 0 else [0.0]*10
                        
                        self.current_batch_stats = {
                            'lambda_t': lambda_t,
                            'ablation_phi_bound': ablation_phi_bound,
                            'indomain_token_count': total_indomain_tokens,
                            'total_valid_token_count': total_valid_tokens,
                            'indomain_token_ratio': indomain_ratio,
                            'phi_mean': phi_valid.mean().item(),
                            'phi_std': phi_valid.std().item(),
                            'phi_min': phi_valid.min().item(),
                            'phi_max': phi_valid.max().item(),
                            'phi_median': phi_valid.median().item(),
                            'gamma_mean': gamma_valid.mean().item(),
                            'gamma_std': gamma_valid.std().item(),
                            'gamma_min': gamma_valid.min().item(),
                            'gamma_max': gamma_valid.max().item(),
                            'gamma_median': gamma_valid.median().item(),
                        }
                        for i, ratio in enumerate(gamma_bin_ratios):
                            self.current_batch_stats[f'gamma_bin_{i}_ratio'] = ratio
                        for i, ratio in enumerate(phi_bin_ratios):
                            self.current_batch_stats[f'phi_bin_{i}_ratio'] = ratio
            else:
                ...
                entropy = -(probs * torch.log(probs + 1e-10)).sum(dim=-1)
                log_prob = -loss_fct(logits_rmpad, input_ids_rmpad_rolled)
                phi_t = log_prob + entropy
                ...
                token_probs = torch.gather(probs, dim=1, index=input_ids_rmpad_rolled.unsqueeze(-1)).squeeze(-1)
                epsilon = 1e-10
                token_probs = torch.clamp(token_probs, epsilon, 1.0)
                
                polylog_loss_val = -(token_probs ** gamma_t.detach()) * torch.log(token_probs)
                
                indomain_mask_unpad = ((phi_t > -ablation_phi_bound) & (phi_t < ablation_phi_bound)).float()
                
                loss = polylog_loss_val
                loss = gather_outputs_and_unpad(loss, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                indomain_mask_gathered = gather_outputs_and_unpad(indomain_mask_unpad, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                ...
                full_indomain_mask = pad_input(
                    hidden_states=indomain_mask_gathered.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                )
                full_indomain_mask = full_indomain_mask.squeeze(-1)[:, :-1].reshape(-1)
                
                loss_mask = loss_mask.to(full_loss.device)
                combined_mask = loss_mask * full_indomain_mask.detach()
                loss = full_loss * combined_mask
                
                if self.device_mesh.get_rank() == 0 and self.config.ulysses_sequence_parallel_size <= 1:
                    with torch.no_grad():
                        try:
                            phi_full = pad_input(
                                hidden_states=phi_t.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                            ).squeeze(-1)[:, :-1].reshape(-1)
                            gamma_full = pad_input(
                                hidden_states=gamma_t.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                            ).squeeze(-1)[:, :-1].reshape(-1)
                            valid_tokens = loss_mask.bool()
                            indomain_tokens = (combined_mask > 0).bool()
                            gamma_valid = gamma_full[valid_tokens]
                            phi_valid = phi_full[valid_tokens]
                            ...
                            logger.warning(f"Failed to collect stats with sequence parallelism: {e}")
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                if rank == 0:
                    ...
                    if hasattr(self, 'current_batch_stats') and self.current_batch_stats:
                        stats_entry = {'step': global_step, **self.current_batch_stats}
                        self.weight_stats_history.append(stats_entry)
                        
                        import json
                        with open(self.stats_save_path, 'a') as f:
                            f.write(json.dumps(stats_entry) + '\n')
                        
                        stats_for_tb = {f'indomain/{k}': v for k, v in self.current_batch_stats.items()}
                        tracking.log(data=stats_for_tb, step=global_step)
