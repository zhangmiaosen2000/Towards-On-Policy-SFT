...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            # Fix: Patch model_config to handle non-serializable objects (same as save_model.config)
            original_model_config_to_json_string = model_config.to_json_string
            
            def patched_model_config_to_json_string(use_diff=True):
                ...
                cleaned_dict = deep_clean(config_dict)
                if cleaned_dict is None or not cleaned_dict:
                    cleaned_dict = {"_note": "Config was empty or contained only non-serializable values"}
                
                return json.dumps(cleaned_dict, indent=2, sort_keys=False) + "\n"
            
            model_config.to_json_string = patched_model_config_to_json_string
            
            model_config.save_pretrained(hf_config_tokenizer_path)
            
            # Restore original method
            model_config.to_json_string = original_model_config_to_json_string
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                # Fix: Monkey patch config save to handle None values and non-serializable objects
                # Phi-4 config may contain None values and function objects
                if hasattr(save_model, 'config') and save_model.config is not None:
                    # Save original method
                    original_to_json_string = save_model.config.to_json_string
                    
                    # Create patched method that handles all edge cases
                    def patched_to_json_string(use_diff=True):
                        # Get config dict
                        config_dict = save_model.config.to_dict()
                        
                        # Deep clean: remove None and non-serializable objects
                        def deep_clean(obj):
                            if obj is None:
                                return None
                            # Skip non-serializable types (functions, classes, etc.)
                            elif callable(obj) or isinstance(obj, type):
                                return None
                            elif isinstance(obj, dict):
                                result = {}
                        ...
                        # Use sort_keys=False to avoid None comparison issues
                        return json.dumps(cleaned_dict, indent=2, sort_keys=False) + "\n"
                    
                    # Apply the patch
                    save_model.config.to_json_string = patched_to_json_string
                    
                    if self.rank == 0:
                        print(f"Applied comprehensive config save patch for {save_model.config.model_type}")
                
                # Now save with the patched config
                save_model.save_pretrained(hf_local_path, state_dict=state_dict, safe_serialization=True)
                
                # Restore original method
                if 'original_to_json_string' in locals():
                    save_model.config.to_json_string = original_to_json_string
                log_with_rank(
