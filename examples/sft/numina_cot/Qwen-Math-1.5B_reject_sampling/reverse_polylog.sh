...
nproc_per_node=8
project_name=RJ-reverse-polylog

experiment_name=RJ-reverse-polylog-qwen-2.5-math-1.5B-2
save_path=/mnt/vast/data/miaosen/lys/checkpoints/$experiment_name

torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node \
        -m verl.trainer.fsdp_reverse_polylog_trainer \
    ...
    model.reverse_polylog_beta=1.0 \
    model.reverse_polylog_gamma_min=0.1 \
    model.reverse_polylog_gamma_max=2.0 \
    model.reverse_polylog_warmup_ratio=0.2 \
    model.reverse_polylog_transition_ratio=0.1 \
