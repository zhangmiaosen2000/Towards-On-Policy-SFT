    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                token_probs = torch.softmax(shift_logits, dim=-1).gather(1, shift_labels.unsqueeze(-1)).squeeze(-1).detach()
                loss = loss * token_probs
                ...
                token_probs = torch.softmax(logits_rmpad, dim=-1).gather(1, input_ids_rmpad_rolled.unsqueeze(-1)).squeeze(-1).detach()
                loss = loss * token_probs
