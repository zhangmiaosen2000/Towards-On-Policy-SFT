...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        beta = getattr(self.config.model, 'reverse_polylog_beta', 1.0)
        gamma_min = getattr(self.config.model, 'reverse_polylog_gamma_min', 0.1)
        gamma_max = getattr(self.config.model, 'reverse_polylog_gamma_max', 2.0)
        warmup_ratio = getattr(self.config.model, 'reverse_polylog_warmup_ratio', 0.2)
        transition_ratio = getattr(self.config.model, 'reverse_polylog_transition_ratio', 0.1)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                epsilon = 1e-10
                token_probs = torch.clamp(token_probs, epsilon, 1.0 - epsilon)
                
                reverse_weight = ((1.0 - token_probs) ** gamma_t).detach()
                reverse_polylog_loss_val = reverse_weight * (-torch.log(token_probs))
                
                loss = reverse_polylog_loss_val * loss_mask.to(reverse_polylog_loss_val.device)
