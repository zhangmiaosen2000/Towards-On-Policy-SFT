...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        clip_B = getattr(self.config.model, 'indomain_clip_B', 10.0)
        alpha_max = getattr(self.config.model, 'indomain_alpha_max', 2.0)
        alpha_min = getattr(self.config.model, 'indomain_alpha_min', 0.1)
        warmup_ratio = getattr(self.config.model, 'indomain_warmup_ratio', 0.2)
        transition_ratio = getattr(self.config.model, 'indomain_transition_ratio', 0.1)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                phi_clip = torch.clamp(phi_t, -clip_B, clip_B)
                r_t = torch.clamp(-phi_clip / clip_B, 0.0, 1.0)
                log_alpha_raw = (1 - r_t) * torch.log(torch.tensor(alpha_max)) + r_t * torch.log(torch.tensor(alpha_min))
                alpha_raw = torch.exp(log_alpha_raw)
                
                alpha_t = 1.0 + lambda_t * (alpha_raw - 1.0)
                ...
                alpha_detached = alpha_t.detach()
                f_alpha = torch.where(
                    torch.abs(alpha_detached - 1.0) < epsilon,
                    -torch.log(token_probs),
                    -(torch.pow(token_probs, alpha_detached) - 1) / alpha_detached
                )
                
                loss = f_alpha * loss_mask.to(f_alpha.device)
                
                if self.device_mesh.get_rank() == 0:
                    with torch.no_grad():
                        valid_tokens = loss_mask.bool()
                        alpha_valid = alpha_t[valid_tokens]
                        phi_valid = phi_t[valid_tokens]
                        r_t_valid = r_t[valid_tokens]
                        
                        alpha_bins = torch.linspace(alpha_min, alpha_max, 11)
                        alpha_bin_counts = torch.histogram(alpha_valid.cpu(), bins=alpha_bins.cpu())[0]
                        alpha_bin_ratios = (alpha_bin_counts / alpha_bin_counts.sum()).tolist() if alpha_bin_counts.sum() > 0 else [0.0]*10
                        
                        phi_bins = torch.linspace(-clip_B, clip_B, 11)
                        ...
                        self.current_batch_stats = {
                            ...
                            'alpha_std': alpha_valid.std().item(),
                            'alpha_min': alpha_valid.min().item(),
                            'alpha_max': alpha_valid.max().item(),
                            'alpha_median': alpha_valid.median().item(),
                            'r_t_mean': r_t_valid.mean().item(),
                            'r_t_std': r_t_valid.std().item(),
                            'r_t_median': r_t_valid.median().item(),
                        }
                        for i, ratio in enumerate(alpha_bin_ratios):
                            self.current_batch_stats[f'alpha_bin_{i}_ratio'] = ratio
                ...
                if self.device_mesh.get_rank() == 0 and self.config.ulysses_sequence_parallel_size <= 1:
                    with torch.no_grad():
                        try:
                            ...
                            alpha_full = pad_input(
                                hidden_states=alpha_t.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                            ).squeeze(-1)[:, :-1].reshape(-1)
                            r_t_full = pad_input(
                                hidden_states=r_t.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                            ).squeeze(-1)[:, :-1].reshape(-1)
                            valid_tokens = loss_mask.bool()
                            alpha_valid = alpha_full[valid_tokens]
                            phi_valid = phi_full[valid_tokens]
                            r_t_valid = r_t_full[valid_tokens]
