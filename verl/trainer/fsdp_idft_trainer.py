...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        tau = getattr(self.config.model, 'indomain_tau', 0.0)
        temperature = getattr(self.config.model, 'indomain_temperature', 1.0)
        w_min = getattr(self.config.model, 'indomain_w_min', 0.1)
        w_max = getattr(self.config.model, 'indomain_w_max', 1.0)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                weight_raw = w_max - (w_max - w_min) * sigmoid_val
                
                weight = 1.0 + lambda_t * (weight_raw - 1.0)
                
                loss = loss_fct(shift_logits, shift_labels) * weight.detach()
                loss = loss * loss_mask.to(loss.device)
                
                if self.device_mesh.get_rank() == 0:
                    with torch.no_grad():
                        valid_tokens = loss_mask.bool()
                        weight_valid = weight[valid_tokens]
                        
                        bins = torch.linspace(w_min, w_max, 11)
                        bin_counts = torch.histogram(weight_valid.cpu(), bins=bins.cpu())[0]
                        bin_ratios = (bin_counts / bin_counts.sum()).tolist()
                        
                        self.current_batch_stats = {
                            'lambda_t': lambda_t,
                            'phi_mean': phi_t[valid_tokens].mean().item(),
                            'phi_std': phi_t[valid_tokens].std().item(),
                            'phi_min': phi_t[valid_tokens].min().item(),
                            'phi_max': phi_t[valid_tokens].max().item(),
                            'weight_mean': weight_valid.mean().item(),
                            'weight_std': weight_valid.std().item(),
                            'weight_min': weight_valid.min().item(),
                            'weight_max': weight_valid.max().item(),
                        }
                        for i, ratio in enumerate(bin_ratios):
                            self.current_batch_stats[f'weight_bin_{i}_ratio'] = ratio
            else:
                ...
                loss = loss_fct(logits_rmpad, input_ids_rmpad_rolled) * weight.detach()
                ...
                if self.device_mesh.get_rank() == 0 and self.config.ulysses_sequence_parallel_size <= 1:
                    with torch.no_grad():
                        try:
                            ...
                            weight_full = pad_input(
                                hidden_states=weight.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                            ).squeeze(-1)[:, :-1].reshape(-1)
                            valid_tokens = loss_mask.bool()
                            weight_valid = weight_full[valid_tokens]
                            ...
                            self.current_batch_stats = {
                                'lambda_t': lambda_t,
                                'phi_mean': phi_full[valid_tokens].mean().item(),
                                'phi_std': phi_full[valid_tokens].std().item(),
                                'phi_min': phi_full[valid_tokens].min().item(),
                                'phi_max': phi_full[valid_tokens].max().item(),
                                'weight_mean': weight_valid.mean().item(),
                                'weight_std': weight_valid.std().item(),
                                'weight_min': weight_valid.min().item(),
                                'weight_max': weight_valid.max().item(),
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                if rank == 0:
                    ...
                    if hasattr(self, 'current_batch_stats'):
